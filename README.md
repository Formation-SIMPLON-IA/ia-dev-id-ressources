...
| [`galerie_genai/`](./galerie_genai/) | **Étagère async optionnelle — à faire APRÈS ton cas d'usage certif** — RAG de bout en bout et son **évaluation** (hit@k, citations, questions pièges, seuil d'abstention), menu de dépannage RAG mesuré (normalisation, HyDE, reranking, chunking sémantique, fusion 🟠, juge d'ancrage 🟠), **agent from scratch** (boucle ReAct, outils typés, 5 garde-fous, prompt injection démontrée puis bornée) et **LoRA sur Colab**. Tout en local via Ollama, `MOCK_MODE` si Ollama est impossible. Prolonge le bonus RAG de M3-B1 et les conceptions M7-B2/M8-B2. Pas de livrable. |
